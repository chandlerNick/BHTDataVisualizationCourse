...
plot_ly(loadings_df, type = 'scatter3d', mode = 'text') %>%
  add_trace(data = segments_df, 
            x = ~x, y = ~y, z = ~z, 
            split = ~varname,
            line = list(color = 'blue')) %>%
  add_trace(data = loadings_df,
            x = ~PC1, y = ~PC2, z = ~PC3,
            type = 'scatter3d', mode = 'text',
            text = ~varname,
            textposition = "top center",
            showlegend = FALSE) %>%
  layout(title = '3D PCA Loadings (Vectors from Origin)',
         scene = list(
...
cor(na.omit(numeric_df_normalized))
```
...
```{r}
colnames(trim_df)
gdp_df <- trim_df[,c("Country", "Real.GDP.per.capita.USD")]

plot_country_data(gdp_df, "Country", "Real.GDP.per.capita.USD", legend_title = "Real GDP per Capita")
```
...
### Arable Land
```{r}
al_df <- trim_df[,c("Country", "Arable.land.percent")]

plot_country_data(al_df, "Country", "Arable.land.percent", legend_title = "Proportion of arable land")
```


### IMR
```{r}
imr_df <- trim_df[,c("Country", "Infant.deaths.per.1000.live.births")]
imr_df$Infant.deaths.per.1000.live.births

plot_country_data(imr_df, "Country", "Infant.deaths.per.1000.live.births", legend_title = "IMR (deaths per 1000 births)")
```

### MMR
```{r}
mmr_df <- trim_df[,c("Country", "Maternal.deaths.per.100000.live.births")]
mmr_df$Maternal.deaths.per.100000.live.births <- mmr_df$Maternal.deaths.per.100000.live.births * 1000

plot_country_data(mmr_df, "Country", "Maternal.deaths.per.100000.live.births", legend_title = "Maternal Deaths per 1000 Births")
```


### Birth Rate
```{r}
br_df <- trim_df[,c("Country", "Live.births.per.1000.people")]
br_df$Live.births.per.1000.people <- br_df$Live.births.per.1000.people*1000

plot_country_data(br_df, "Country", "Live.births.per.1000.people", legend_title = "Births per 1000 people")
```


### Death Rate
```{r}
dr_df <- trim_df[,c("Country", "Deaths.per.1000.people")]
dr_df$Deaths.per.1000.people <- dr_df$Deaths.per.1000.people*1000

plot_country_data(dr_df, "Country", "Deaths.per.1000.people", legend_title = "Deaths per 1000 people")
```
